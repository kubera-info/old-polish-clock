...
//--------------------------------------------------------------------------------------
static HRESULT LoadTextureDataFromFile( BYTE* pBuffer, const DWORD pSize, BYTE** ppHeapData,
                                        DDS_HEADER** ppHeader,
...
static HRESULT CreateTextureFromDDS( ID3D11Device* pDev, DDS_HEADER* pHeader, __inout_bcount(BitSize) BYTE* pBitData,
                                     UINT BitSize, __out ID3D11ShaderResourceView** ppSRV, __out ID3D11Texture2D** ppTex, bool bSRGB )
{
    HRESULT hr = S_OK;
    ...
    if( SUCCEEDED( hr ) && pTex2D )
    {
        ...
        DXUT_SetDebugName( pTex2D, "DDSTextureLoader" );
#endif
        ...
        hr = pDev->CreateShaderResourceView( pTex2D, &SRVDesc, ppSRV ); // Tu do spr.
        DXUT_SetDebugName( *ppSRV, "DDSTextureLoader-Res" );
        //SAFE_RELEASE( pTex2D );
		*ppTex = pTex2D;
    }
...
HRESULT CreateDDSTextureFromFile9( LPDIRECT3DDEVICE9 pDev, BYTE* pBuffer, const DWORD pSize, LPDIRECT3DTEXTURE9* ppTex, DWORD * dwWidth, DWORD * dwHeight )
{
    ...
    HRESULT hr = LoadTextureDataFromFile( pBuffer, pSize, &pHeapData, &pHeader, &pBitData, &BitSize );
    if(FAILED(hr))
    {
	...
	*dwHeight = pHeader->dwHeight;
	*dwWidth = pHeader->dwWidth;
...
HRESULT CreateDDSTextureFromFile11( ID3D11Device* pDev, BYTE* pBuffer, const DWORD pSize, ID3D11ShaderResourceView** ppSRV )
{
	...
	hr = CreateTextureFromDDS( pDev, pHeader, pBitData, BitSize, ppSRV, &pTex, false );
    SAFE_DELETE_ARRAY( pHeapData );
